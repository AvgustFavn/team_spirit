...
async def handle_uber_callback(callback_query: types.CallbackQuery):
    ...
    await bot.send_message(chat_id=user_id,
                           text=f'Чтобы создать ссылку введите в таком формате(то есть каждый параметр с новой строки):\n'
                                f'убер(не менять)\n'
                                f'Цена такси для мамонта в рублях\n'
                                f'Откуда отъезд\n'
                                f'Куда\n')
...
@dp.message_handler(lambda message: 'убер' in message.text.lower())
async def new_link_uber(message: types.Message):
    domene = session.query(Domenes).filter(Domenes.category == 'убер', Domenes.work == True).order_by(asc(Domenes.id)).first()
    user_id = message.from_user.id
    ...
    to_ = lines[3]
    if domene:
        invite_link = f'http://{domene.link}/order/{user_id}{randint(100, 999)}'
        link = Links(user_id=user_id, link=invite_link, category='антикино', price=price, to_address=to_, from_address=from_)
        session.add(link)
        session.commit()
        await bot.send_message(chat_id=user_id, text=f'Ваша новая ссылка: {invite_link}')
    else:
        await bot.send_message(chat_id=user_id, text=f'Видимо доменов для антикино нету :(')
...
async def admin(message: types.Message):
    ...
    if is_admin(int(user_id)):
        ...
        keyboard_markup.add(types.InlineKeyboardButton('Домены', callback_data='domenes'))
        try:
...
@dp.callback_query_handler(lambda c: c.data == 'domenes')
async def domenes_menu(callback_query: types.CallbackQuery):
    user_id = callback_query.from_user.id
    keyboard_markup = types.InlineKeyboardMarkup(row_width=1)
    keyboard_markup.add(types.InlineKeyboardButton('Сменить домен uber', callback_data='change_uber'))
    await bot.send_message(chat_id=user_id, text='Смена доменов', reply_markup=keyboard_markup)

@dp.callback_query_handler(lambda c: c.data == 'change_uber')
async def change_uber(callback_query: types.CallbackQuery):
    user_id = callback_query.from_user.id
    domene = session.query(Domenes).filter(Domenes.category == 'убер', Domenes.work == True).order_by(asc(Domenes.id)).first()
    if domene:
        domene.work = False
        session.add(domene)
        session.commit()
        await bot.send_message(chat_id=user_id, text='Вы сменили домен убер, теперь ссылки будут создаваться на другом домене')
    else:
        await bot.send_message(chat_id=user_id, text='Видимо доменов для убера нету :(')
